...
namespace Acrya.Renderer
{
    internal static partial class Renderer
    {
        ...
        public static void RequestImageLoad(string path, string? name = null)
        {
            ...
            imageLoadRequests.Enqueue((path, name ?? Path.GetFileName(path)));
        }
        ...
        private static void LoadImages()
        {
            ...
            {
                string[] dividedPath = path.Split(Path.DirectorySeparatorChar);
                // Get Image name
                StringBuilder builder = new StringBuilder();
                for (int i = directoriesToImages; i < dividedPath.Length; i++)
                {
                    builder.Append(dividedPath[i]);
                    if (i != dividedPath.Length - 1) { builder.Append(Path.DirectorySeparatorChar); }
                }
